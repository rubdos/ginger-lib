...
impl<F: PrimeField> ConstraintSynthesizer<F> for TestCircuit1<F> {
    fn generate_constraints<CS: ConstraintSystemAbstract<F>>(
        ...
        let mut b_k_minus_1 = FpGadget::<F>::alloc_input(
            cs.ns(|| "alloc b"),
...
fn bench_prover_circuit1(c: &mut Criterion){
    ...
    for &num_constraints in num_constraints.iter()
        {
            let params = {
                let c = TestCircuit1::<Fr>{ num_constraints, a: None, b: None };
            ...
            group.bench_with_input(BenchmarkId::from_parameter(num_constraints), &num_constraints, |bn, _constraints| {
                bn.iter_batched(
                    ...
                    |(a, b)| {
                    let c = TestCircuit1{ num_constraints, a: Some(a), b: Some(b) };
...
fn bench_prover_circuit2(c: &mut Criterion){
    ...
    for &num_constraints in num_constraints.iter()
        {
            let params = {
                let c = TestCircuit2::<Fr>{ num_constraints, a: None, b: None };
                generate_random_parameters::<Bn382, _, _>(c, &mut rng).unwrap()
            };
            ...
            group.bench_with_input(BenchmarkId::from_parameter(num_constraints), &num_constraints, |bn, _constraints| {
                bn.iter_batched(
                    ...
                    |(a, b)| {
                        let c = TestCircuit2{ num_constraints, a: Some(a), b: Some(b) };
