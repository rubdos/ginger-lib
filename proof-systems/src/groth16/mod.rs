            ...
            .and_then(|p| {
                if p.is_zero() {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "invalid point A: point at infinity",
                    ));
                }
                Ok(p)
            })?;
        ...
        let b = E::G2Affine::read_checked(&mut reader)
            ...
            .and_then(|p| {
                if p.is_zero() {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "invalid point B: point at infinity",
                    ));
                }
                Ok(p)
            })?;
        ...
        let c = E::G1Affine::read_checked(&mut reader)
            ...
            .and_then(|p| {
                if p.is_zero() {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "invalid point C: point at infinity",
                    ));
                }
                Ok(p)
            })?;
...
fn read_affine_vec_checked<G: AffineCurve, R: Read>(
    len: usize,
    zero_check: bool,
    mut reader: R,
) -> IoResult<Vec<G>> {
            ...
            for (j, &p2) in gamma_abc_g1.iter().skip(i + 1).enumerate() {
                if p1 == p2 {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        format!(
                            "duplicate points: gamma_abc_g1[{}] = gamma_abc_g1[{}]",
                            i, j
                        ),
                    ));
                }
...
impl<E: PairingEngine> SemanticallyValid for VerifyingKey<E> {
    #[inline]
    fn is_valid(&self) -> bool {
        self.alpha_g1_beta_g2.is_valid()
            && !self.alpha_g1_beta_g2.is_zero()
            && self.gamma_g2.is_valid()
    ...
    fn read_checked<R: Read>(mut reader: R) -> IoResult<Self> {
        let alpha_g1_beta_g2 = E::Fqk::read_checked(&mut reader)
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid alpha_g1_beta_g2: {}", e),
                )
            })
            .and_then(|f| {
                if f.is_zero() {
        ...
        let gamma_g2 = E::G2Affine::read_checked(&mut reader)
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid point gamma_g2: {}", e),
                )
            })
            .and_then(|p| {
                if p.is_zero() {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "invalid point gamma_g2: point at infinity",
                    ));
                }
                Ok(p)
            })?;
        ...
        let delta_g2 = E::G2Affine::read_checked(&mut reader)
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid point delta_g2: {}", e),
                )
            })
            .and_then(|p| {
                if p.is_zero() {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "invalid point delta_g2: point at infinity",
                    ));
                }
                Ok(p)
            })?;
        ...
        let gamma_abc_g1 = read_affine_vec_checked::<E::G1Affine, _>(ic_len, true, &mut reader)
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid gamma_abc_g1: {}", e),
                )
            })?;
        Self::check_gamma_abc_g1_points(gamma_abc_g1.as_slice()).map_err(|e| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("invalid gamma_abc_g1: {}", e),
            )
...
impl<E: PairingEngine> FromBytes for VerifyingKey<E> {
    ...
    fn read<R: Read>(mut reader: R) -> IoResult<Self> {
        let alpha_g1_beta_g2 =
            E::Fqk::read(&mut reader).map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
...
impl<E: PairingEngine> SemanticallyValid for Parameters<E> {
    ...
    fn is_valid(&self) -> bool {
        self.vk.is_valid()
            && self.alpha_g1.is_valid()
            && !self.alpha_g1.is_zero()
            && self.beta_g1.is_valid()
            && !self.beta_g1.is_zero()
            && self.beta_g2.is_valid()
            && !self.beta_g2.is_zero()
            && self.delta_g1.is_valid()
    ...
    fn read_checked<R: Read>(mut reader: R) -> IoResult<Self> {
        ...
        let alpha_g1 = E::G1Affine::read_checked(&mut reader)
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid point alpha_g1: {}", e),
                )
            })
            .and_then(|p| {
                if p.is_zero() {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "invalid point alpha_g1: point at infinity",
                    ));
                }
                Ok(p)
            })?;
        ...
        let beta_g1 = E::G1Affine::read_checked(&mut reader)
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid point beta_g1: {}", e),
                )
            })
            .and_then(|p| {
                if p.is_zero() {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "invalid point beta_g1: point at infinity",
                    ));
                }
                Ok(p)
            })?;
        ...
        let beta_g2 = E::G2Affine::read_checked(&mut reader)
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid point beta_g2: {}", e),
                )
            })
            .and_then(|p| {
                if p.is_zero() {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "invalid point beta_g2: point at infinity",
                    ));
                }
                Ok(p)
            })?;
        ...
        let delta_g1 = E::G1Affine::read_checked(&mut reader)
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid point delta_g1: {}", e),
                )
            })
            .and_then(|p| {
                if p.is_zero() {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "invalid point delta_g1: point at infinity",
                    ));
                }
                Ok(p)
            })?;
        ...
        let b_g2_query = read_affine_vec_checked::<E::G2Affine, _>(b_g2_len, false, &mut reader)
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid b_g2_query: {}", e),
                )
            })?;

        let h_len = reader.read_u32::<BigEndian>()? as usize;
        let h_query =
            read_affine_vec_checked::<E::G1Affine, _>(h_len, true, &mut reader).map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid h_query: {}", e),
                )
            })?;

        let l_len = reader.read_u32::<BigEndian>()? as usize;
        let l_query =
            read_affine_vec_checked::<E::G1Affine, _>(l_len, true, &mut reader).map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("invalid l_query: {}", e),
                )
            })?;
